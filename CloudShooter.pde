import java.awt.*;//importar biblioteca gráfica
...
{ //codigo apenas executado no inicio do programa
    video = new Capture(this, 320, 180, 30);    
    video.start();
    surface.setTitle("GayVota by Cláudio & Mariana & Sílvia"); //titulo da janela
    ...
    surface.setSize(1920, 1080 /*screenWidth, screenHfeight*/);
        ...
        pm = newPlayerShipMenu(width/2, height/2); //menu para escolha de "nave" usa bala e imagem diferente
        c1 = newCloudsGen("assets/images/cloud1.png", 100, random(height)); //nuvem 1
        c2 = newCloudsGen("assets/images/cloud2.png", 200, random(height)); //nuvem 2
        c3 = newCloudsGen("assets/images/cloud3.png", 300, random(height)); //nuvem 3
        c4 = newCloudsGen("assets/images/cloud4.png", 400, random(height)); //nuvem 4
        c5 = newCloudsGen("assets/images/cloud5.png", 500, random(height)); //nuvem 5
        p1 = newPlayer("assets/images/first_ship_cs.png", -200, height/2); //player 1 //spawn fora do canvas para animar a entrada do player no jogo
        e1 = newArrayList<Enemy>(); //enemy 1 (necessario tornar isto num array list de waves para attack)
    ...
    { //player menu = escolha de cor para a nave (desbloqueia com x score)
    pm.drawme();
    pm.back.drawme();
    pm.ship1.drawme();
    pm.ship2.drawme();
    ...
    if(p1.moveUnLock)
    {
    //p1.posX = map(faces[0].x, 0, 320, 0, height);
    //p1.posY = map(faces[0].y, 0, 180, 0, height);
}
    m.background.drawme();
    ...
    voidhealthcheck() 
    { //verificar se o player colidiu com o inimigo // passar isto para o codigo do player noutra funcao e correr dentro da funcao movimento. // tambem preciso de colocar as colisoes a funcionar em forma de retangulo (mais simples de implementar do que poligonos ou ellipse)
